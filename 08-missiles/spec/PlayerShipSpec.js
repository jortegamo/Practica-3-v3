describe("Clase PlayerShip", function(){
	...
	it ("Step con la tecla fire pulsada estando no pulsada", function(){
		var miBoard = new GameBoard();
		Game = {width: 320, height: 480};
		miNave = new PlayerShip();
		miBoard.add (miNave); // para que miNave referencie a board.
		spyOn (miBoard, "add");
		// nos aseguramos de que pulsamos cuando no estaba pulsada.
		var dt = 1;
		...
		miNave.step(dt);
		runs(function(){
			expect (miBoard.add).toHaveBeenCalled();
		});
